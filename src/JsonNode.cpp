...
json_node::json_node(const json_node& n) {
    this->json_free();
    this->json_copy(&n);
}

json_node& json_node::operator=(const json_node& n) {
    this->json_free();
    this->json_copy(&n);
    return *this;
}
...
static int json_parse_value(json_node* node, json_context* c) {
    switch (*(c->json)) {
        case '\0':
            return JSON_PARSE_EXPECT_VALUE;
        case 'n':
            return json_parse_null(node, c);
        case 't':
            return json_parse_true(node, c);
        case 'f':
            return json_parse_false(node, c);
        case '\"':
            return json_parse_string(node, c);
        case '[':
            return json_parse_array(node, c);
        case '{':
            return json_parse_object(node, c);
        default:
            return json_parse_number(node, c);
    }
}
...
bool json_node::json_get_bool() const {
    assert(this->type == JSONTYPE_TRUE || this->type == JSONTYPE_FALSE);
...
double json_node::json_get_number() const {
    assert(this->type == JSONTYPE_NUMBER);
    return this->num;
}
...
std::string json_node::json_get_string() const {
    assert(this->type == JSONTYPE_STRING);
    return this->str;
}
...
void json_node::json_set_array(std::vector<json_node*> a) {
    this->arr = a;
    this->type = JSONTYPE_ARRAY;
}

int json_node::json_get_array_size() const {
    assert(this->type == JSONTYPE_ARRAY);
...
void json_node::json_set_object(std::vector<std::pair<std::string, json_node*>> o) {
    this->obj = o;
    this->type = JSONTYPE_OBJECT;
}

int json_node::json_get_object_size() const {
    assert(this->type == JSONTYPE_OBJECT);
    return this->obj.size();
}

std::string json_node::json_get_object_key(int index) const {
    assert(this->type == JSONTYPE_OBJECT && index >= 0 && index < this->obj.size());
    int i = 0;
...
void json_node::json_set_object_value(std::string key, json_node* v) {
    ...
    this->obj.emplace_back(std::pair<std::string, json_node*>(key, v));
}
...
void json_node::json_pushback_object_element(const std::string& key, json_node* v) {
    assert(this->type == JSONTYPE_OBJECT);
    this->obj.emplace_back(std::pair<std::string, json_node*>(key, v));
}
...
static void json_stringify_string(const json_node* node, std::string& str) {
    ...
    assert(node->json_get_type() == JSONTYPE_STRING);
    str.push_back('"');
...
static void json_stringify_value(const json_node* node, std::string& str) {
    switch (node->json_get_type()) {
        ...
        case JSONTYPE_NUMBER:
            json_stringify_number(node, str);
            break;
        case JSONTYPE_STRING:
            json_stringify_string(node, str);
            break;
        case JSONTYPE_ARRAY:
            str += '[';
            for (int i = 0; i < node->json_get_array_size(); i++) {
        ...
        case JSONTYPE_OBJECT:
            ...
            for (int i = 0; i < node->json_get_object_size(); i++) {
                if (i > 0)
                    str += ',';
                str += '"';
                str += node->json_get_object_key(i);
                str += '"';
                str += ':';
...
void json_node::json_copy(const json_node* src) {
    int i;
    assert(src != this);
